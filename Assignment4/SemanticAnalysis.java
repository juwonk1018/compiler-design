...
public class SemanticAnalysis implements Visitor {
  ...
  public void check(Program progAST) {
    ...
    /* Start of your code: */
    Decl main = scopeStack.retrieve("main");
    if(main == null){
  ...
  public void visit(FunDecl x) {
    ...
    /* Start of your code: */
    boolean notExist = scopeStack.enter(x.idAST.Lexeme,x);
    if(!notExist){
  ...
  public void visit(FormalParamDecl x) {
    ...
    /* Start of your code: */
    boolean notExist = scopeStack.enter(x.astIdent.Lexeme, x);
    if(!notExist){
  ...
  public void visit(VarDecl x) {
    ...
    if (!(x.eAST instanceof EmptyExpr)) {
      ...
      if (x.tAST instanceof ArrayType) {
        ...
        /* Start of your code: */
        Type arrayT = ((ArrayType)x.tAST).astType;
        Expr iter = x.eAST;
        int seqNr = 0;
        if(iter instanceof ExprSequence){
          while(iter instanceof ExprSequence){
            seqNr++;
            if(((ExprSequence)iter).lAST.type.AssignableTo(arrayT))
            {
              if(arrayT.Tequal(StdEnvironment.floatType) &&  ((ExprSequence)iter).lAST.type.Tequal(StdEnvironment.intType)){
                ((ExprSequence)iter).lAST = i2f(((ExprSequence)iter).lAST);
              }
            }
            else
            {
              reporter.reportError(errMsg[13], "", x.idAST.pos);
              return;
            }
            iter = ((ExprSequence)iter).rAST;
          }
        }
        else{
          reporter.reportError(errMsg[15], "", iter.pos);
          return;
        }
        Expr E = ((ArrayType)x.tAST).astExpr;
        assert (E instanceof IntExpr);
        int arrayNr = ((IntExpr)E).GetValue();
        if(arrayNr < seqNr){
          reporter.reportError(errMsg[16], "", iter.pos);
          return;
        }
        /* End of your code */
      } else {
        ...
        if(x.eAST instanceof ExprSequence){
          reporter.reportError(errMsg[14], "", x.eAST.pos);
          return;
        }
        
        else{
          if(x.eAST.type.AssignableTo(scalarT)){
            if(scalarT.Tequal(StdEnvironment.floatType) &&  x.eAST.type.Tequal(StdEnvironment.intType)){
              x.eAST = i2f(x.eAST);
            }
          }
          else{
  ...
  public void visit(CallExpr x) {
    ...
    int nrFormal = GetNrOfFormalParams(F);
    int nrActual = GetNrOfActualParams(x);
    
    if(nrFormal < nrActual){
